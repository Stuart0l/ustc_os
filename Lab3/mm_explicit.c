/*
 * mm-explicit.c - The explicit free linklist implementation of malloc.
 * 
 * allocated block: the same structure as the implicit implementation
 * free block: has pred and succ pointer pointing to neighbor free block
 *
 */
#include <stdio.h>
#include <stdlib.h>
...
#define FTRP(bp) ((char *)(bp) + GET_SIZE(HDRP(bp)) - 2*DSIZE)  // free blk
...
static void *extend_heap(size_t words) {
    ...
    PUT(HDRP(bp), PACK(size, 0));  // set head
    PUT(FTRP(bp), GET(HDRP(bp)));  // set foot
    PUT(HDRP(NEXT_BLKP(bp)), PACK(0, 1));  // set end
...
int mm_init(void)
{
    if ((head_listp = mem_sbrk(6*WSIZE)) == (void*)-1)
        return -1;
    PUT(head_listp, 0);
    PUT(head_listp + (1*WSIZE), PACK(2*DSIZE, 1));
    PUT(head_listp + (4*WSIZE), PACK(2*DSIZE, 1));
    PUT(head_listp + (5*WSIZE), PACK(0, 1));  // end blk
    head_listp += (4*WSIZE);
...
void *mm_malloc(size_t size)
{
    ...
    if(size <= DSIZE)
        ...
        asize = 2*DSIZE * ((size + DSIZE + (2*DSIZE-1))/(2*DSIZE));  // blk size must be multiple of 4 words
